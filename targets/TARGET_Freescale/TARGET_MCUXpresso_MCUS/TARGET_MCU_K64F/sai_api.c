...
const sai_format_t k64f_pcm_mode = {
    .sample_rate = 16000,
    .bclk_polarity = true,
    .wclk_polarity = true,
    .ws_delay = false,
...
bool sai_init(sai_t *obj, sai_init_t *init) {
    ...
    if (!init->TX.enable && !init->RX.enable) {
        ...
        printf("!init->TX.enable && !init->RX.enable \n");
        return false;
    }
    if (init->TX.enable && memcmp(&(init->TX.format), &k64f_pcm_mode, sizeof(sai_format_t)) != 0) {
        // we only support 1 format so far
        printf("memcmp(&(init->TX.format), &k64f_pcm_mode, sizeof(sai_format_t)) != 0 \n");
        return false;
    }
    ...
    if (init->TX.enable) {
        ...
        switch(init->sync) {
            case sai_synchronicity_async:
                config.syncMode = kSAI_ModeAsync;
            break;

            case sai_synchronicity_synced_to_tx:
            case sai_synchronicity_synced_to_rx:
        ...
        //config.bclkSource = kSAI_BclkSourceBusclk;
    ...
    if (init->RX.enable) {
        ...
        //config.mclkSource = kSAI_MclkSourceSysclk;
        config.bclkSource = kSAI_BclkSourceOtherSai1;
...
uint32_t sai_xfer(sai_t *obj, uint32_t channel, uint32_t sample) {
    ...
    if (obj->writable) {
        ...
        if (SAI_TxGetStatusFlag(obj->base) & kSAI_FIFOErrorFlag) {
            return 0xffffffff;
        }
    ...
    if (!obj->readable) { return 0xabababab; }
    
    SAI_RxEnable(obj->base, true);

    while (!(SAI_RxGetStatusFlag(obj->base) & kSAI_FIFORequestFlag)) {
       //printf("RX status: %x \n", SAI_RxGetStatusFlag(obj->base));
    }
