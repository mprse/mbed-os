...
static sai_result_t stm_sai_init(sai_t *obj, sai_init_t *init) {
    ...
    printf("PLLSAIRDY : %sset\r\n", __HAL_RCC_PLLSAI_GET_FLAG()?"":"re");
    ...
    printf("Source Clock = %lu (%s); m = %lu\r\n", (uint32_t)(((RCC->PLLCFGR & RCC_PLLCFGR_PLLSRC) == RCC_PLLSOURCE_HSI)?HSI_VALUE:HSE_VALUE),
                                                   (((RCC->PLLCFGR & RCC_PLLCFGR_PLLSRC) == RCC_PLLSOURCE_HSI)?"HSI":"HSE"),
                                                   (uint32_t)(RCC->PLLCFGR & RCC_PLLCFGR_PLLM));
    printf("vcoinput = %lu; q = %lu; n = %lu\r\n", vcoinput, tmpreg, (uint32_t)((RCC->PLLSAICFGR & RCC_PLLSAICFGR_PLLSAIN) >> 6U));
    
    /* SAI_CLK_x = SAI_CLK(first level)/PLLSAIDIVQ */
    tmpreg = (((RCC->DCKCFGR & RCC_DCKCFGR_PLLSAIDIVQ) >> 8U) + 1U);
    printf("qdiv = %lu\r\n", (uint32_t)tmpreg);
    sai_clock_source = sai_clock_source/(tmpreg); 
    printf("sai_clock_source = %lu (%lu)\r\n", sai_clock_source, HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_SAI_PLLSAI));
    ...
    if (mckdiv == 0) {
        ...
        printf("required_mclk = %lu > sai_clock_source\r\n", required_mclk);
        return SAI_RESULT_GENERIC_FAILURE;
    }
    if (mckdiv >= 32) {
        // out of MCKDIV range, reduce sai_clock_source frenquency
        printf("mckdiv = %lu is out of register's range [1-32)\r\n", mckdiv);
        return SAI_RESULT_GENERIC_FAILURE;
    }
    printf("mckdiv = %lu required_mclk = %lu mclk = %lu bclk = %lu\r\n",
            mckdiv, required_mclk, sai_clock_source/mckdiv, bitclock);
