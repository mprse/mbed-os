...
void test_get_module()
{
    ...
    spi_name = spi_get_module(SPI_MASTER_MOSI, SPI_MASTER_MISO, SPI_CLK);

    TEST_ASSERT_EQUAL(SPI_BASE, spi_name);

    spi_name = spi_get_module(SPI_SLAVE_MOSI, SPI_MASTER_MISO, SPI_CLK);
...
void test_get_capabilities()
{
    ...
    /* Slave capabielities. */
    spi_get_capabilities(spi_get_module(SPI_MASTER_MOSI, SPI_MASTER_MISO, SPI_CLK), NC, &capabilities);
    ...
    spi_get_capabilities(spi_get_module(SPI_MASTER_MOSI, SPI_MASTER_MISO, SPI_CLK), SPI_CS, &capabilities);
...
void test_init_free()
{
    ...
    spi_get_capabilities(spi_get_module(SPI_SLAVE_MOSI, SPI_SLAVE_MISO, SPI_CLK), SPI_CS, &capabilities);

    /* SPI master - CS pin ignored. */
    /* Full duplex. */
    spi_init(&spi_obj, false, SPI_MASTER_MOSI, SPI_MASTER_MISO, SPI_CLK, NC);

    spi_free(&spi_obj);

    /* Half duplex: MASTER <--- SLAVE. */
    spi_init(&spi_obj, false, NC, SPI_MASTER_MISO, SPI_CLK, NC);

    spi_free(&spi_obj);

    /* Half duplex: MASTER ---> SLAVE. */
    spi_init(&spi_obj, false, SPI_MASTER_MOSI, NC, SPI_CLK, NC);

    spi_free(&spi_obj);

    /* SPI master - SS pin defined. */
    /* Full duplex. */
    spi_init(&spi_obj, false, SPI_MASTER_MOSI, SPI_MASTER_MISO, SPI_CLK, SPI_CS);

    spi_free(&spi_obj);

    /* Half duplex: MASTER <--- SLAVE. */
    spi_init(&spi_obj, false, NC, SPI_MASTER_MISO, SPI_CLK, SPI_CS);

    spi_free(&spi_obj);

    /* Half duplex: MASTER ---> SLAVE. */
    spi_init(&spi_obj, false, SPI_MASTER_MOSI, NC, SPI_CLK, SPI_CS);
    ...
    if (capabilities.support_slave_mode) {
        ...
        /* Full duplex. */
        spi_init(&spi_obj, true, SPI_SLAVE_MOSI, SPI_SLAVE_MISO, SPI_CLK, SPI_CS);

        spi_free(&spi_obj);

        /* Half duplex: MASTER <--- SLAVE. */
        spi_init(&spi_obj, true, NC, SPI_SLAVE_MISO, SPI_CLK, SPI_CS);

        spi_free(&spi_obj);

        /* Half duplex: MASTER ---> SLAVE. */
        spi_init(&spi_obj, true, SPI_SLAVE_MOSI, NC, SPI_CLK, SPI_CS);
