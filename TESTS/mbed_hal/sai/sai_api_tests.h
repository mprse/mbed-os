 ...
 * When SAI is initialised with undefined(NULL) parameter.
 ...
 *
 * Given is platform with SAI support.
 * When the device can never support the specified configuration.
 * Then `SAI_RESULT_CONFIG_UNSUPPORTED` initialisation status is returned.
 *
 * @note:
 * Provide some invalid configurations which should be caught by the driver.
 */
void sai_init_unsupported_config_test(void);
 ...
 *
 * @note:
 * This test requires wire loopback connection to be executed.
 */
 ...
 * When the loopback communication test is performed using obligate format.
