...
static void sai_config_create(sai_init_t *p_config, bool is_receiver, uint32_t sample_rate, sai_format_t * p_format)
{
    if (is_receiver) {
...
static loopback_test_tatus_t sai_loopback_communication_test(sai_format_t *format, uint32_t sample_rate)
{
    ...
    if (transmitter_init_status == SAI_RESULT_OK &&
        receiver_init_status == SAI_RESULT_OK) {
        ...
        while (1) {
            if (write_cnt < BUFFER_SIZE) {
                ...
                if (sai_xfer(&sai_transmitter, &sample) == true) {
                    write_cnt++;
                }
            }

            if (sai_xfer(&sai_receiver, &sample) == true) {
...
void sai_transmission_test() {
    ...
    if (!LOOPBACK_CONNECTION) {
        printf("NO LOOPBACK CONNECTION - TEST SKIPPED! \n");
        return;
    }
    ...
    for (uint32_t f_idx = 0; f_idx < (sizeof(test_sai_formats)/sizeof(sai_format_t)); f_idx++) {
        for (uint32_t sr_idx = 0; sr_idx < (sizeof(test_sample_rates)/sizeof(uint32_t)); sr_idx++) {
            const loopback_test_tatus_t result = sai_loopback_communication_test(&test_sai_formats[f_idx], test_sample_rates[sr_idx]);

            /* First `required_formats_count` formats are required, so we expect that
...
void sai_reinit_test()
{
    ...
    sai_config_create(&sai_init_data, false, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16w32);

    TEST_ASSERT_EQUAL(SAI_RESULT_OK, sai_init(&sai_obj, &sai_init_data));

    /* Change config (format) and try to re-init. */
    sai_config_create(&sai_init_data, false, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16);
...
/* Test that sai_init() returns `SAI_RESULT_CONFIG_UNSUPPORTED` if the device can never support
 * the specified configuration. */
void sai_init_unsupported_config_test()
{
    sai_t sai_obj = { };
    sai_init_t sai_init_data = { };

    /* Create invalid configuration: invalid sample rate. */
    sai_config_create(&sai_init_data, false, 0, (sai_format_t*)&sai_mode_i2s16w32);

    TEST_ASSERT_EQUAL(SAI_RESULT_CONFIG_UNSUPPORTED, sai_init(&sai_obj, &sai_init_data));

    /* Create invalid configuration: invalid pin. */
    sai_config_create(&sai_init_data, false, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16w32);
    sai_init_data.mclk = LED1;
    ...
    /* Create invalid configuration: invalid format (word select length equal to 0). */
    sai_config_create(&sai_init_data, false, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16w32);
    sai_init_data.format.ws_length = 0;
...
void sai_reinit_after_free_test()
{
    ...
    sai_config_create(&sai_init_data, true, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16);
...
void sai_xfer_receiver_invalid_param_test()
{
    sai_t sai_obj = { };
    sai_init_t sai_init_data = { };

    sai_config_create(&sai_init_data, true, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16w32);

    TEST_ASSERT_EQUAL(SAI_RESULT_OK, sai_init(&sai_obj, &sai_init_data));

    uint32_t sample = 0xABABABAB;

    const bool result = sai_xfer((sai_t *)NULL, &sample);
...
void sai_xfer_receiver_empty_fifo_test()
{
    sai_t sai_obj = { };
    sai_init_t sai_init_data = { };
    ...
    const bool result = sai_xfer(&sai_obj, &sample);
...
static void sai_xfer_receiver_psample_test()
{
    if (!LOOPBACK_CONNECTION) {
        printf("NO LOOPBACK CONNECTION - TEST SKIPPED! \n");
        return;
    ...
    sai_config_create(&sai_transmitter_init, false, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16w32);
    sai_config_create(&sai_receiver_init, true, SAI_DEFAULT_SAMPLE_RATE, (sai_format_t*)&sai_mode_i2s16w32);
    ...
    while (1) {
        ...
        if (read_cnt & 1) {
            if (sai_xfer(&sai_receiver, NULL) == true) {
                read_cnt++;
 ...
 * the `sai_t` object is NULL. */
void sai_xfer_transmitter_invalid_param_test()
{
    sai_t sai_obj = { };
    sai_init_t sai_init_data = { };
...
static void sai_xfer_transmitter_psample_test()
{
    if (!LOOPBACK_CONNECTION) {
        printf("NO LOOPBACK CONNECTION - TEST SKIPPED! \n");
        return;
    ...
    while (1) {
        if (write_cnt < BUFFER_SIZE) {
            ...
            if (write_cnt & 1) {
                if (sai_xfer(&sai_transmitter, NULL) == true) {
                    write_cnt++;
                }
            } else {
                sample = SAMPLE_1;
        ...
        if (sai_xfer(&sai_receiver, &sample) == true) {
            TEST_ASSERT_EQUAL((read_cnt & 1) ? 0 : SAMPLE_1, sample);
            read_cnt++;
        }
...
Case cases[] = {
      ...
      Case("sai_init() - unsupported config.", sai_init_unsupported_config_test),
      ...
      Case("receiver: sai_xfer() - invalid param", sai_xfer_receiver_invalid_param_test),
      Case("receiver: sai_xfer() - empty FIFO", sai_xfer_receiver_empty_fifo_test),
      Case("receiver: sai_xfer() - psample defined/undefined", sai_xfer_receiver_psample_test),

      Case("transmitter: sai_xfer() - invalid param", sai_xfer_transmitter_invalid_param_test),
      Case("transmitter: sai_xfer() - psample defined/undefined", sai_xfer_transmitter_psample_test),
