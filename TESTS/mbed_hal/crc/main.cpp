 ...
 * is supported, false otherwise (at least one polynomial must be supported).  */
void crc_is_supported_test()
{
    /* Check if at least one crc polynomial is supported. */
...
void crc_calc_multi_test()
{
    ...
    for (int i = 0; i < (sizeof(test_cases) / sizeof(TEST_CASE)); i++) {
        if (hal_crc_is_supported(&test_cases[i].config_data) == true) {
            ...
            const uint32_t third_part_bytes = strlen((const char*) test_cases[i].input_data) - first_part_bytes - second_part_bytes;
            ...
            hal_crc_compute_partial((uint8_t*) (test_cases[i].input_data + first_part_bytes + second_part_bytes), third_part_bytes);
            crc = hal_crc_get_result();
...
{
    uint32_t crc = 0;
    int pol_idx[SUP_POL_LEN] = { UNSUPPORTED, UNSUPPORTED };
    int pol_cnt = 0;
    ...
    /* Change index only if more than one polynomial is supported. */
    if (pol_idx[SUP_POL_LEN - 1] != UNSUPPORTED) {
        pol_cnt++;
    }
