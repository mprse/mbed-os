...
static void test_spi_transfer()
{
    ...
    for (int i = 0; i < test_cases[tc_id].slave_rx_cnt; i++) {
        p_rx_pattern_slave[i] &= symbol_mask;
    }

    for (int i = 0; i < test_cases[tc_id].master_rx_cnt; i++) {
...
#if DEBUG
    printf("Test context:\n");
    printf("Master: tx_sym_cnt: %u tx:%.*s rx_sym_cnt: %u\n", master_ctx.tx_len, master_ctx.tx_len * sizeof(T), master_ctx.tx, master_ctx.rx_len);
    printf("Slave:  tx_sym_cnt: %u tx:%.*s rx_sym_cnt: %u\n", slave_ctx.tx_len, slave_ctx.tx_len * sizeof(T), slave_ctx.tx, slave_ctx.rx_len);
    printf("Ran in %d ms (cnt = %llu) : %f\n", t.read_ms(), cnt, (float)cnt / (float)t.read_ms());
    printf("Slave  sent  : %.*s\n", slave_ctx.tx_len * sizeof(T), tx_slave);
    printf("Master recved: %.*s\n", master_ctx.rx_len * sizeof(T), rx_master);
    printf("Master sent  : %.*s\n", master_ctx.tx_len * sizeof(T), tx_master);
    printf("Slave  recved: %.*s\n", slave_ctx.rx_len * sizeof(T), rx_slave);
#endif
...
Case cases[] = {
    ...
    Case("SPI master-slave async com - freq testing: min defined", test_spi_transfer<uint8_t, 15, false>),
    Case("SPI master-slave async com - freq testing: max defined", test_spi_transfer<uint8_t, 16, false>),
    Case("SPI master-slave async com - master: TX > RX", test_spi_transfer<uint8_t, 17, false>),
    Case("SPI master-slave async com - master: TX < RX", test_spi_transfer<uint8_t, 18, false>),
    Case("SPI master-slave async com - slave: TX > RX", test_spi_transfer<uint8_t, 19, false>),
    Case("SPI master-slave async com - slave: TX < RX", test_spi_transfer<uint8_t, 20, false>),
    Case("SPI master-slave async com - master: TX undefined", test_spi_transfer<uint8_t, 21, false>),
    Case("SPI master-slave async com - master: RX undefined", test_spi_transfer<uint8_t, 22, false>),
    Case("SPI master-slave async com - slave: TX undefined", test_spi_transfer<uint8_t, 23, false>),
    Case("SPI master-slave async com - slave: RX undefined", test_spi_transfer<uint8_t, 24, false>),
    Case("SPI master-slave async com - master: manual ss", test_spi_transfer<uint8_t, 25, false>),
    Case("SPI master-slave async com - one symbol", test_spi_transfer<uint8_t, 26, true>),
    Case("SPI master-slave async com - long buffer", test_spi_transfer<uint8_t, 27, true>),
    * /
#endif
};
