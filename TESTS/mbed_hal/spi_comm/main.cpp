...
/* 1. Overview
 *
 * This is SPI communication test which verifies various SPI configurations.
 ...
 * This command needs to be executed with BUILD_MASTER flag set to 1.
 *
 ...
 * This command needs to be executed with BUILD_MASTER flag set to 0 and then
 * target must be flushed manually.
 *
 * 2. What is verified by the test?
 *    a) master/slave mode,
 *    b) full duplex/half duplex modes,
 *    c) synchronous/asynchronous modes,
 *    d) symbol sizes: [1, 32],
 ...
 *       status to master and test case is skipped.
...
/* 1 - build MASTER part of the test.
 * 2 - build SLAVE part of the test.
 */
...
/* Function returns true if configuration is consistent with capabilities of the SPI peripheral,
 * false otherwise. */
...
static config_test_case_t test_cases[] = {
        /* default config: 8 bit symbol\sync mode\full duplex\clock idle low\sample on the first clock edge\MSB first\1 MHz clock\automatic SS handling */
...
void test_transfer_master()
{
    ...
    if (check_capabilities(&capabilities, test_cases[tc_id].symbol_size, false, test_cases[tc_id].duplex)) {
        ...
        spi_format(&spi_master, test_cases[DEFAULT_CFG].symbol_size, test_cases[DEFAULT_CFG].mode,
        ...
        if (status == CONFIG_STATUS_OK) {
            ...
            init_transmission_buffers<T>(tc_id, &tx_pattern[0], &rx1_pattern[0], &rx2_pattern[0], &tx_buff[0],
                    &rx_buff[0], &fill_symbol);

            dump_buffers<T>(tx_pattern, rx1_pattern, rx2_pattern, tx_buff, rx_buff);

            wait_before_transmission();
            handle_ss(ss_pin, true);
            count = spi_transfer(&spi_master, p_tx_buff, test_cases[tc_id].master_tx_cnt, p_rx_buff,
                    test_cases[tc_id].master_rx_cnt, (void*) &fill_symbol);
            handle_ss(ss_pin, false);
            ...
            wait_before_transmission();
            handle_ss(ss_pin, true);
            count = spi_transfer(&spi_master, tx_buff, TEST_SYM_CNT, rx_buff, TEST_SYM_CNT, (void*) &fill_symbol);
            handle_ss(ss_pin, false);
...
void slave_transfer(spi_t *obj, config_test_case_t *config)
{
    ...
    count = spi_transfer(obj, p_tx_buff, TEST_SYM_CNT, p_rx_buff, TEST_SYM_CNT, &fill_symbol);
...
void print_config(config_test_case_t *config)
{
    Timer tim;
    tim.reset();
    tim.start();
...
int main()
{
    ...
    while (true) {
        ...
        if (check_capabilities(&capabilities, config.symbol_size, true, config.duplex)) {
            ...
            spi_format(&spi_slave, config.symbol_size, (spi_mode_t) config.mode,
                    (spi_bit_ordering_t) config.bit_ordering);
