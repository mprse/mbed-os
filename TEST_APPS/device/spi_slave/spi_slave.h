        ...
        T *p_tx_buff, T *p_rx_buff, T *p_fill_symbol)
{
    ...
    set_pattern(p_tx_pattern, config->slave_tx_cnt, sizeof(T), tx_pattern, sizeof(tx_pattern));
    set_pattern(p_rx1_pattern, config->slave_rx_cnt, sizeof(T), rx_pattern, sizeof(rx_pattern));
    set_pattern(p_rx2_pattern, config->slave_rx_cnt, sizeof(T), tx_pattern, sizeof(tx_pattern));
...
int test_transfer_slave(spi_t * obj, config_test_case_t *config)
{
    ...
    { obj, config, NULL, CMDLINE_RETCODE_SUCCESS };
