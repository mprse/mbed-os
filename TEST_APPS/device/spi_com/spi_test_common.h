...
static int check_capabilities(config_test_case_t *tc_config, bool slave)
{
    ...
    if (slave) {
        spi_get_capabilities(spi_get_module(MASTER_SPI_MOSI, MASTER_SPI_MISO, MASTER_SPI_CLK), MASTER_SPI_SS, false, &capabilities);
    } else {
        spi_get_capabilities(spi_get_module(SLAVE_SPI_MOSI, SLAVE_SPI_MISO, SLAVE_SPI_CLK), SLAVE_SPI_SS, true, &capabilities);
    }
