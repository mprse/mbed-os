...
/* Function inserts symbol as string into the message. */
 ...
 *
 * . */
static void set_pattern(void * p_buffer, uint32_t sym_count, uint32_t sym_size, uint8_t * p_pattern,
...
/* Function inits specified buffer with given pattern. */
template<typename T>
static void set_symbol(T * p_sym, uint8_t pattern)
{
    if (p_sym == NULL) {
        return;
    }
    ...
    T sym_pattern;
    set_buffer(&sym_pattern, pattern);
