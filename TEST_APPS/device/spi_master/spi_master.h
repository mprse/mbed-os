        ...
        T *p_tx_buff, T *p_rx_buff, T *p_fill_symbol)
{
    ...
    set_pattern(p_tx_pattern, sym_count, sizeof(T), tx_pattern, sizeof(tx_pattern));
    set_pattern(p_rx1_pattern, sym_count, sizeof(T), rx_pattern, sizeof(rx_pattern));
    set_pattern(p_rx2_pattern, sym_count, sizeof(T), tx_pattern, sizeof(tx_pattern));
...
int test_transfer_master(spi_t * obj, config_test_case_t *config, DigitalOut * ss)
{
    ...
    trans_thread_params_t trans_thread_params =
    { obj, config, ss, CMDLINE_RETCODE_SUCCESS };

    /* Set slave transmission test timeout to 5 sec + master delays */
    transm_thread_timeout.attach_us(transm_timeout_handler, 5 * US_PER_S + 2 * TRANSMISSION_DELAY_MS * US_PER_MS);
